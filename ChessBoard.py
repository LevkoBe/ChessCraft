...
class ChessBoard:
    def __init__(self, rows: int, columns: int, board: List[List[ChessBoardPiece]] = None):
        ...
        self.board: List[List[ChessBoardPiece]] = board if board else [[None for _ in range(columns)] for _ in
                                                                       range(rows)]
    ...
    def move_piece(self, selected_square: Tuple[int, int], white_pieces: List[Tuple[str, int, int]], \
                   black_pieces: List[Tuple[str, int, int]], clicked_row: int, clicked_col: int) -> Tuple[
        list[Tuple[str, int, int]], Tuple[list[Tuple[str, int, int]]]]:
        ...
        if current_piece.color == 'w':
            white_pieces = [
                (piece[0], clicked_row, clicked_col) if piece[1] == selected_square[0] and piece[2] == selected_square[
                    1]
    ...
    def find_best_move(self, white_pieces: List[Tuple[str, int, int]], black_pieces: List[Tuple[str, int, int]],
                       piece_mapping: PieceMapping, player_turn):
        move_to_value = {}
        maximal_depth = 3

...
class GameFlow:
    ...
    def play_game(self, maximum_moves=-1, coefficients=None):
        if not coefficients:
            coefficients = self.game.coefficients
        ...
        while self.running and moves_played != maximum_moves:
            ...
            if current_bot:
                ...
                self.player_select_and_move(ui, current_player, coefficients)
        ...
        pygame.quit()
        return self.game.board.evaluate_position(self.white_pieces, self.black_pieces, self.game.piece_mapping, self.game.coefficients)
    
    def player_select_and_move(self, ui, current_player, coefficients):

        def player_select_piece():
            while True:
                row, col, cheat = ui.get_mouse_click()
                self.cheat(cheat, current_player, coefficients)
    ...
    def cheat(self, cheatcode, player_turn, coefficients):
        if cheatcode == "hint":
